...
|---|---|
| Plateformes | Développement et validation intermédiaire Web/Electron sur W18 uniquement. Cible finale : Electron Windows x64 et macOS arm64 sur M1 ; Web Chrome/Firefox sur W18 et M1, Safari sur M1. Qualification M1 uniquement aux phases finales 14 à 16. |
| Matériel | W18, PC Windows 10 de génération 2018, est le seul poste du développement (étapes 0 à 13). MacBook M1 intervient uniquement aux tests finaux 14 à 16. Mac Intel (MI) est retiré du périmètre par D04-04 ; aucun Mac n'est requis pour le PASS de 0.4. |
...
**Règle de plateforme D04-04 du 22 septembre 2026 :** jusqu'à la fin de l'étape 13, les critères d'acceptation s'exécutent sur W18 uniquement. Toute mention de « chaque cible », « chaque plateforme » ou de matrice complète dans ces étapes désigne ce périmètre de développement ; les scénarios sont conservés pour leur rejeu final sur M1 en 14 à 16. Aucun relevé, appareil, navigateur Safari, installateur ou essai natif Mac n'est un prérequis du PASS/Go de 0.4 ou d'un jalon de développement. La CI technique existante peut conserver ses runners : elle ne constitue pas une cible matérielle supplémentaire. Mac Intel est hors périmètre ; M1 reste à qualifier réellement en fin de parcours, sans PASS anticipé. Voir la [décision tracée](docs/measurement/DECISIONS.md).
...
**D04-04, décision utilisateur du 22 septembre 2026, remplace D04-03 :** développement uniquement sur W18 ; qualification Mac uniquement sur M1 et uniquement en phases finales 14 à 16. Mac Intel est retiré. T05/H-M1 sont hors Go 0.4 et reportés à ces phases ; H-MI est retiré. L'absence de M1 ou d'Intel ne bloque aucun PASS de l'étape en cours. H-W18 reste un contrôle distinct ; ses observations physiques inconnues ne sont pas transformées en PASS par cette décision de plateforme. Voir la [décision et le retour humain](docs/measurement/DECISIONS.md).
...
| M1 — phases finales 14 à 16 uniquement | MacBook M1, RAM/stockage/OS exacts à relever au début des tests finaux ; aucune configuration supposée acquise. Son absence ne bloque pas les étapes 0 à 13. |
| Web | Chrome et Firefox sur W18 pendant le développement ; Chrome, Firefox et Safari réel sur M1 uniquement en phases 14 à 16. Qualifier les versions stables retenues et la précédente supportée, avec numéros inscrits au rapport de chaque campagne. |
| Desktop | Windows x64 natif sur W18 pendant le développement ; macOS arm64 natif sur M1 aux phases finales. Aucun installateur ni test Mac Intel requis. |
...
Le support technique d'un OS et son cycle de sécurité sont distingués. La version exacte d'Electron doit rester maintenue et compatible avec W18 et, lors des tests finaux, le macOS du M1 retenu. Windows 10 standard a quitté le support Microsoft le 14 octobre 2025 ; consigner l'édition et sa couverture de maintenance sans supprimer W18. [Plateformes Electron](https://github.com/electron/electron#platform-support), [maintenance Electron](https://www.electronjs.org/docs/latest/tutorial/electron-timelines), [cycle Windows](https://learn.microsoft.com/en-us/windows/release-health/release-information).
...
| 0.4 Fixer corpus et budgets | Relever W18 ; préparer l'outillage M1 pour les seuls tests finaux 14 à 16 (D04-04), sans exiger de Mac maintenant. Fixer OS/navigateurs, tailles et données ; adopter ou ajuster avant implémentation les cibles de la section 5. Séparer corpus de référence et fichiers personnels. | Dix contrôles obligatoires dont H-W18 ; rapport reproductible, chaque budget associé à un script ou protocole ; charges générables sans manuscrit confidentiel. L'absence de M1 ou d'Intel n'est pas un obstacle au PASS/Go 0.4 ; Mac Intel est hors périmètre, aucun PASS Mac n'est anticipé. |
...
| 1.3 Démarrer Web et Electron hors ligne | Prototype installé Electron Windows et application Web préparée pour offline sur W18 ; ressources, polices, routes, codecs requis et workers locaux. Définir navigation desktop et repli des routes Web. | Sur W18, après préparation, fermer l'application/le navigateur, couper le réseau, redémarrer l'OS, ouvrir une route directe et utiliser le prototype sans serveur de développement. Aucun Mac requis ; scénario conservé pour rejeu M1 en 14 à 16. |
| 1.4 Raccorder une infrastructure privée pilote | Comparer un petit nombre de solutions adaptées au stockage d’un éditeur/auteur : authentification, coûts, exploitation, transferts reprenables et accès navigateur. Choisir un premier connecteur, sans imposer tous les fournisseurs. | Sur W18, écrire/lire un fichier synthétique depuis Chrome, Firefox et Electron Windows ; tester CORS, HTTPS, expiration d'accès et coupure ; tracer le chemin réseau et démontrer qu'aucun contenu ne passe par Jaquette Cloud. Rejeu M1, dont Safari, aux phases finales 14 à 16. |
...
| 3.5 Faire le parcours vertical | Sélectionner un mot, associer un vrai son local, écouter, autosauvegarder, fermer, relancer hors ligne et retrouver exactement l’association. | Scénario bloquant sur W18 (Chrome, Firefox et Electron Windows) ; média source rendu indisponible ; ancre et lecture intactes. Rejeu sur M1, dont Safari et Electron macOS arm64, uniquement en phases finales 14 à 16. |
...
| 9.6 Actualiser les tableaux de bord | Rafraîchissement connecté, dernière actualisation, état périmé et reprise progressive. Notifications minimales sans contenu sensible. | Deux sessions isolées sur W18 observent le même état confirmé dans le budget prévu ; hors ligne l'écran indique sa date ; travail local non envoyé n'apparaît pas comme reçu par l'équipe. Une panne d'infrastructure privée ne corrompt pas le cache. Rejeu entre W18 et M1 aux phases finales 14 à 16. |
...
**Jalon J2 :** deux contributeurs et trois Réviseurs dans des sessions/profils/instances isolés sur W18, avec identités et stockages séparés, accomplissent deux cycles complets avec coupures réseau par session et une révocation, sans perte ni duplication. Les scénarios entre postes physiques W18 et M1 sont rejoués aux phases finales 14 à 16 ; aucun second poste n'est requis pour J2.
...
| 11.3 Télécharger et intégrer | Cache contrôlé, téléchargement reprenable, vérification d’empreinte, copie dans projet à l’utilisation. | Coupure puis reprise ; fichier incomplet jamais utilisable ; son placé, banque coupée et projet rouvert dans une autre instance isolée sur W18, sans accès au cache ni aux sources de la première : lecture intacte. Rejeu sur M1 aux phases finales 14 à 16. |
...
| 12.3 Connexion du navigateur | Appairage explicite à Desktop, origine et workspace limités ; exposition des seules ressources autorisées de la session Web. Gérer fermeture/gel de l’onglet. | Chrome et Firefox sur W18 échangent une proposition ; autre site, autre workspace, appairage expiré et onglet indisponible refusés ; Desktop ne lit pas arbitrairement le stockage d'un navigateur. Rejeu M1, dont Safari, aux phases finales 14 à 16. |
...
La préparation technique de cette étape peut suivre ses prérequis, mais la campagne matérielle M1 débute uniquement après J3, pendant les phases finales 14 à 16. Elle ne constitue jamais un prérequis anticipé des étapes 0 à 13. Mac Intel est hors périmètre.
...
| 14.2 Distribuer Desktop | Début des essais Mac, sur M1 uniquement : relever matériel/OS/navigateurs et conditions, exécuter le collecteur natif. Builds Windows x64 et macOS arm64 sur M1, installateurs signés, notarisation et ticket attaché macOS ; associations de fichiers. | Installation sur W18 et M1 propres ; signature vérifiée ; ouverture .jacq ; premier lancement M1 hors ligne ouvre l'application et explique l'activation requise ; après activation, parcours local complet. Sans M1, qualification finale macOS BLOCKED, sans invalider les PASS de développement W18. Aucun Mac Intel requis. |
...
| Sous-étape | Travail et livrable | Test d’acceptation |
|---|---|---|
| 15.1 Mesurer toute la matrice | Rejouer la section 5 et les scénarios des étapes 1 à 13 sur W18 et M1 seulement, builds distribués et moteurs réels ; relevé natif M1 acquis en 14.2. Playwright Chromium/Firefox/WebKit complète mais ne remplace pas Safari réel sur M1. | Résultats par poste/navigateur et cache ; aucune cellule W18/M1 obligatoire omise, aucune cellule Intel exigée ; les cibles adoptées en 0.4 sont tenues ou la release finale reste non qualifiée. Les PASS des étapes de développement W18 restent distincts. |
