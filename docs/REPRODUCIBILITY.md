...
Mise à jour du 22 septembre 2026 : après régénération de l'EPUB français par Gutenberg, le mapping utilise aussi l'[archive française](../reference-data/ARCHIVE-FR.md). Les références FR et EN ont chacune une archive explicite des octets déjà canoniques ; les quatre autres gardent leur URL d'origine. Aucun hash, taille ou licence du manifeste n'est remplacé.

`pnpm prepare:references` lit le [manifeste canonique](../reference-data/manifest.json), utilise l'[archive documentée](../reference-data/ARCHIVE.md) pour l'anglais, l'archive française et les quatre autres URL d'origine, puis vérifie taille et SHA-256 avant toute création. Seul le manifeste définit les octets attendus ; le [mapping d'acquisition](../scripts/reference-source.mjs) ne les redéfinit pas. Les fichiers présents sont revérifiés ; un fichier invalide est conservé et signalé, jamais écrasé. La préparation poursuit les autres acquisitions pour donner un diagnostic complet, puis termine avec un code non nul si une ressource manque ou diffère. Aucune autre source n'est essayée après un échec. Les licences, attributions et sources restent dans le manifeste, l'[inventaire](../reference-data/INVENTORY.md) et la notice d'archive. Les binaires restent ignorés sous reference-data/files.
...
La copie canonique anglaise a été publiée sans modification en asset de Release de fixtures, distincte d'une version applicative et hors de l'historique Git. Son téléchargement anonyme est vérifié ; la qualification finale exige encore l'acquisition complète et la CI au SHA concerné. Ne pas substituer un fichier de EPUB tests/, réécrire les attentes ou déclarer le lot acquis sur un simple succès local.
...
- `pnpm test:tooling` utilise le corpus canonique dans des copies temporaires : jeu complet, absence, illisibilité, octet altéré, taille, manifestes invalides. Un fetch simulé uniquement dans des processus de test contrôle le choix de l'archive, le téléchargement conforme, HTTP 404, réseau indisponible, octet altéré, troncature, dépassement de taille et fichier local invalide : vrais codes de sortie, absence de fallback et aucun écrasement. Il vérifie aussi YAML/YML et la propagation d'échec de la vraie chaîne de commandes, avec commandes terminales simulées dans un package temporaire ; le cas référence manquante exécute le vrai vérificateur. La campagne vierge et la CI téléchargent les vrais fichiers sur le réseau.
...
La concurrence est limitée au workflow entier : le groupe combine son nom et le numéro de PR, ou la ref de branche pour un push hors PR. `cancel-in-progress: true` annule les anciennes exécutions actives et remplace celle en attente quand une nouvelle arrive dans le même groupe. Les autres PR, branches et workflows restent indépendants. Les déclencheurs restent PR vers main et push main : un commit de PR ne lance pas aussi un deuxième workflow de push. Les runs terminés et leurs preuves restent consultables ; aucune suppression de l'historique n'est programmée. Les étapes de collecte respectent `!cancelled()` afin de ne pas prolonger une qualification remplacée. Voir la [validation de non-concurrence](validation/ci-non-concurrence/VALIDATION_REPORT.md) et le [contrat GitHub](https://docs.github.com/en/actions/how-tos/write-workflows/choose-when-workflows-run/control-workflow-concurrency).
